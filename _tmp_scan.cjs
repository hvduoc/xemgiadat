...
const code=lines.slice(0,120).join("\n");
let state="normal";let line=1,col=0,startLine=0,startCol=0;
for(let i=0;i<code.length;i++){
  ...
  if(state==="normal"){
    ...
    else if(ch==="/" && code[i+1]==="/") {state="line-comment";startLine=line;startCol=col;}
    else if(ch==="/" && code[i+1]==="*") {state="block-comment";startLine=line;startCol=col;}
...
if(state!=="normal"){
  ...
  console.log("Strings OK in first 120 lines");
}
